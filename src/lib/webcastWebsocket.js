...
function _handleEvents2() {
	this.on("connect", (wsConnection) => {
		...
		this.pingInterval = setInterval(
			() => _classPrivateMethodGet(this, _sendPing, _sendPing2).call(this),
			10000,
		);
		wsConnection.on("message", (message) => {
			if (message.type === "binary") {
				_classPrivateMethodGet(this, _handleMessage, _handleMessage2).call(
					this,
					message,
				);
			}
...
async function _handleMessage2(message) {
	try {
		let decodedContainer = await deserializeWebsocketMessage(
			message.binaryData,
		);

		if (decodedContainer.id > 0) {
			_classPrivateMethodGet(this, _sendAck, _sendAck2).call(
				this,
				decodedContainer.id,
			);
