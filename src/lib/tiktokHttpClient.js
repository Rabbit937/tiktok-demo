...
class TikTokHttpClient {
	constructor(customHeaders, axiosOptions, sessionId) {
		...
		if (Cookie) {
			Cookie.split("; ").forEach((v) =>
				this.cookieJar.processSetCookieHeader(v),
			);
		}
	...
	async getMainPage(path) {
		let response = await _classPrivateMethodGet(this, _get, _get2).call(
			this,
			`${Config.TIKTOK_URL_WEB}${path}`,
		);
		return response.data;
	}

	async getDeserializedObjectFromWebcastApi(
		path,
		params,
		schemaName,
		shouldSign,
	) {
		let url = await _classPrivateMethodGet(this, _buildUrl, _buildUrl2).call(
			this,
			Config.TIKTOK_URL_WEBCAST,
			path,
			params,
			shouldSign,
		);
	...
	async postFormDataToWebcastApi(path, params, formData) {
		let response = await _classPrivateMethodGet(this, _post, _post2).call(
			this,
			`${Config.TIKTOK_URL_WEBCAST}${path}`,
			params,
			formData,
			"json",
		);
...
async function _buildUrl2(host, path, params, sign) {
	...
	if (sign) {
		fullUrl = await signWebcastRequest(
			fullUrl,
