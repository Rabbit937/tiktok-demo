...
async function signRequest(providerPath, url, headers, cookieJar) {
	...
	try {
		...
		for (signHost of [
			config.signProviderHost,
			...config.signProviderFallbackHosts,
		]) {
			try {
				...
				if (
					...
					typeof signResponse.data === "object"
				) {
					break;
				}
			} catch (err) {
		...
		if (
			!(
				(_signResponse$data = signResponse.data) !== null &&
				_signResponse$data !== void 0 &&
				_signResponse$data.signedUrl
			)
		) {
